...
int main(void){
    ...
    bitTest.si = 7; // Не се принтира 7, поради недостиг на bits (short int e 16 bit минимум, а ние използваме 3)
    bitTest.c = 'c'; // Променя стойноста от 99 (c) до -29 (y) / причината е преливане на типа, char e 8 bit минимум, а ние използваме 6
    printElements(bitTest);
    printf("Size of bitStruct: %d\n",sizeof(bitTest)); // Когато използваме #pragma pack(1) - Големината на структурата е 3 бита, а без препроцесора е 4 бита.

    struct bitStruct1 bitTest1;
    printf("Size of bitStruct1 with four types: %d\n",sizeof(bitTest1)); // Големината се увеличава с 3 бита, когато са подредени
